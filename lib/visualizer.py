...
def ref_inpaint_visualize_html(res, rv_imgs, path, n_col=6):
    """Visualize the reference-based inpainting in HTML.
    Args:
        res: A list of [id_idx, image_idx, mask_id, x_in, x_out, cosim],
            representing the ID name, image name, mask index, input, output,
            cosine similarity, respectively.
        rv_imgs: in (N, 3, H, W), [0, 1] scale.
    """
    n_row = math.ceil(res[-2].shape[0] / n_col)
    page = HtmlPageVisualizer(2 * n_row + 1, n_col)
    for i, (id_idx, image_idx, _, x_in, x_out, cosim) in enumerate(zip(*res)):
        row_idx, col_idx = i // n_col * 2, i % n_col
        page.set_cell(row_idx, col_idx,
            text=f"id={int(id_idx)} image_id={int(image_idx)}",
            image=torch2image(x_in))
        page.set_cell(row_idx + 1, col_idx,
            text=f"cosim={cosim.item():.5f}",
            image=torch2image(x_out))
    if rv_imgs is not None:
        for i in range(rv_imgs.shape[0]):
            page.set_cell(2 * n_row, i,
                image=torch2image(rv_imgs[i], data_range="[0,1]"))
    page.save(path)
